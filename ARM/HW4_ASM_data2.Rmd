...
# start a file
Trans <- file.create('TransactionTweetsFile_data2')
# open the file
Trans <- file('TransactionTweetsFile_data2', open = "a")

## Tokenize to words 
Tokens<-tokenizers::tokenize_words(
  tweet$text[401],stopwords = stopwords::stopwords("en"), 
...
TweetTrans <- read.transactions('TransactionTweetsFile_data2',
                                rm.duplicates = FALSE,
                                format = "basket",
                                sep=","
                                ## cols =
...
```{r}
TweetDF <- read.csv('TransactionTweetsFile_data2', 
                    header = FALSE, sep = ",")
head(TweetDF)
(str(TweetDF))
...
write.table(TweetDF, file = "UpdatedTweetFile_data2.csv", col.names = FALSE, 
            row.names = FALSE, sep = ",")
TweetTrans <- read.transactions("UpdatedTweetFile_data2.csv", sep =",", 
...
networkD3::saveNetwork(D3_network_Tweets, 
                       "NetD3_wildfireL_data2.html", selfcontained = TRUE)
```

Visualize the rules
```{r}
itemsets <- eclat(TweetTrans, parameter = list(support = 0.04, minlen=2))
